...
<html>
	<head>
		...
		<fmt:message bundle="${loc}" key="local.catalog.Sweatshirts" var="Sweatshirts" />
		<fmt:message bundle="${loc}" key="local.catalog.Shorts" var="Shorts" />
		<fmt:message bundle="${loc}" key="local.catalog.Outerwear" var="Outerwear" />
		<fmt:message bundle="${loc}" key="local.catalog.Boots" var="name_Boots" />
		<fmt:message bundle="${loc}" key="local.catalog.Sneakers" var="name_Sneakers" />
		<fmt:message bundle="${loc}" key="local.catalog.Slippers" var="name_Slippers" />
		...
		<fmt:message bundle="${loc}" key="local.catalog.Umbrellas" var="Umbrellas" />
							...
							<c:forEach items="${requestScope.footwearsubcategoryList}"
								...
								<li><a
									href="Controller?command=getgoodslist&gender=${sessionScope.gender}&category=Footwear&subcategory=${footwearsubcategory.name}">
										<c:out value="${footwearsubcategory.name}" /></a></li>
							</c:forEach>
						</ul>
					</div></li>
				<li><a
